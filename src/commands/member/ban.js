...
module.exports = async (client, interaction) => {
  ...
  if (member && !member.permissions.has(PermissionFlagsBits.BanMembers)) {
    return interaction.editReply({
      embeds: [
        embed
          .setDescription(
            `${emojify(false)} | **You don't have emough permissions to use this command.**`,
          )
          .setColor("Red"),
      ],
    });
  ...
  interaction.editReply({
    embeds: [
      embed
        .setDescription(
          `${emojify(true)} | **Checking if this member is bannable**`,
        )
        .setColor("DarkPurple"),
    ],
  });
  ...
  if (guild.ownerId === op.member.id) {
    return interaction.editReply({
      embeds: [
        embed
          .setDescription(
            `${emojify(false)} | **I can't ban the guild owner.**`,
          )
          .setColor("Red"),
      ],
    });
  ...
  if (client.user.id === op.member.id) {
    return interaction.editReply({
      embeds: [
        embed
          .setDescription(`${emojify(false)} | **I can't ban myself.**`)
          .setColor("Red"),
      ],
    });
  }
  ...
  if (op.member.roles.highest.comparePositionTo(member.roles.highest) >= 1) {
    return interaction.editReply({
      embeds: [
        embed
          .setDescription(
            `${emojify(false)} | **${op.member} has a higher role than you.**`,
          )
          .setColor("Red"),
      ],
    });
  ...
  if (!op.member.bannable) {
    return interaction.editReply({
      embeds: [
        embed
          .setDescription(`${emojify(false)} | **I can't ban this member**`)
          .setColor("Red"),
      ],
    });
  }
  ...
  const message = await interaction.editReply({
    embeds: [
      embed
        .setDescription(
          `**All the checks have been completed.**\n- **Are you sure you want to ban ${op.member}?**`,
        )
        .setColor("DarkPurple"),
    ],
    components: [row],
    ...
    try {
      ...
      int.editReply({
        ...
        embeds: [
          embed
            .setDescription(
              `${emojify(true)} | **Banned ${op.member.user.username}** | ${op.reason ? op.reason : "No reason provided"}`,
            )
            .setColor("DarkPurple"),
        ],
      });
