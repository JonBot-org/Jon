const { SlashCommandBuilder, ChatInputCommandInteraction, EmbedBuilder, ActionRowBuilder, ButtonBuilder, ButtonStyle, ComponentType, codeBlock } = require("discord.js");
...
module.exports = {
    ...
    run: async (interaction) => {
        ...
        const members = interaction.client.guilds.cache.reduce((acc, guild) => acc + guild.memberCount);
        ...
        collector.on('collect', async (i) => {
            ...
            let newInformation = embed.data.description += `\n\n> **System Information**\nVersion: ${version}\nLibrary:\n${codeBlock(`@discordjs/${dependencies["discord.js"]}\n@chalk/${dependencies["chalk"]}`)}`;

            const newEmbed = new EmbedBuilder()
            .setAuthor({ name: embed.data.author.name, iconURL: embed.data.author.icon_url })
